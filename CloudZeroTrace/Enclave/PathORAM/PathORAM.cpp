...
#ifdef OC_DEBUG
        ocall_print_string("\t[Trusted/Enclave] Pushing path into stash\n");
#endif
...
#ifdef OC_DEBUG
        ocall_print_string("\t[Trusted/Enclave] Performing real access operation\n");
#endif
...
#ifdef OC_DEBUG
        ocall_print_string("\t[Trusted/Enclave] Uploading new path\n");
#endif
...
void PathORAM::PathORAM_RebuildPath(unsigned char *decrypted_path_ptr, uint32_t data_size, uint32_t block_size, uint32_t leaf, uint32_t level) {
    ...
    for(i = 0; i < d; i++){
        ...
        {				
            ...
            uint32_t jprefix = ShiftBy(getTreeLabel(listptr->serialized_block)+n,i); // getTreeLabel(listptr->serialzied_block) = new_leaf
